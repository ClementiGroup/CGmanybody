...
def Validation(model,valid_x,valid_y):
	...
	for j in range(100):
		...
		U_2BD = model(Fvalid_dist_temp,Fvalid_cos_temp,Fvalid_sin_temp,Fvalid_angle_temp)
		
		y_pred_2BD_temp = torch.autograd.grad(-U_2BD,valid_x_temp,create_graph=True, retain_graph=True)

		y_pred_temp = y_pred_2BD_temp[0]
...
def Training(model,train_loader,valid_x, valid_y, IDX):
	...
	torch.save(model, 'Mymodel2BD'+IDX+'.pt')	# save learnt NN to file

	return validError

torch.manual_seed(seed)
model = Model2BD(Zscore_dist, Zscore_cos, Zscore_sin,BondConst,Width,Depth)
