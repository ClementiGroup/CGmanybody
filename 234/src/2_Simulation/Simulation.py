...
model4BD = torch.load('../../1_CV/Mymodel4BD'+str(modelID)+'.pt') # for transfer learning
...
def Simulate(x0, dt, T, freq, SID):
	...
	for i in range(10-SID+1):
		...
		for t in range(1,int(T/10)):
			...
			y_pred_3BD = torch.autograd.grad(-U_3BD,x_old_tensor,create_graph=True, retain_graph=True)
			y_pred_4BD = torch.autograd.grad(-U_4BD,x_old_tensor,create_graph=True, retain_graph=True)
			...
			force_3BD = np.array(y_pred_3BD[0].detach())
			force_4BD = np.array(y_pred_4BD[0].detach())

			force = force_2BD + force_3BD + force_4BD
			#print(force)
