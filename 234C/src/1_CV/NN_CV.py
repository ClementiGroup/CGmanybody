...
def Validation(model,valid_x,valid_y):
	...
	for j in range(100):
		...
		U_4BD = model(Fvalid_dist_temp,Fvalid_cos_temp,Fvalid_sin_temp,Fvalid_angle_temp)
		...
		y_pred_temp = y_pred_2BD_temp[0] + y_pred_3BD_temp[0] + y_pred_4BD_temp[0]
...
def Training(model,train_loader,valid_x, valid_y, IDX):
	...
	for epoch in range(E):
		...
		for i, data in enumerate(train_loader, 0): # in each epoch, loop over all 1000 example
			...
			U_4BD = model(F_dist,F_cos,F_sin,F_angle)
			...
			predict = y_pred_2BD[0] + y_pred_3BD[0] + y_pred_4BD[0]
	...
	torch.save(model, 'Mymodel4BD'+IDX+'.pt')	# save learnt NN to file

	return validError

torch.manual_seed(seed)
model_temp = Model4BD_ch(Zscore_dist, Zscore_cos, Zscore_sin,BondConst,Width,Depth)
