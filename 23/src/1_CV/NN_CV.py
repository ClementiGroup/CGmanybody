...
model2BD = torch.load('Mymodel2BD3.pt')
...
def Validation(model,valid_x,valid_y):
	...
	for j in range(100):
		...
		U_3BD = model(Fvalid_dist_temp,Fvalid_cos_temp,Fvalid_sin_temp,Fvalid_angle_temp)
		...
		y_pred_temp = y_pred_2BD_temp[0] + y_pred_3BD_temp[0]
...
def Training(model,train_loader,valid_x, valid_y, IDX):
	...
	for epoch in range(E):
		...
		for i, data in enumerate(train_loader, 0): # in each epoch, loop over all 1000 example
			...
			U_3BD = model(F_dist,F_cos,F_sin,F_angle)
			...
			predict = y_pred_2BD[0] + y_pred_3BD[0]
			...
			# test error
			if(True and i%2000 == 0 and i!=0 and (epoch+1)%5==0 ):
	...
	torch.save(model, 'Mymodel3BD'+IDX+'.pt')	# save learnt NN to file

	return validError

torch.manual_seed(seed)
model = Model3BD(Zscore_dist, Zscore_cos, Zscore_sin,BondConst,Width,Depth)
valid_error = Training(model, train_loader, valid_x, valid_y,str(modelID))
